...
class spatial_attention(nn.Module):
    ...
    def forward(self, x):
        x0 = x
        avg_out = torch.mean(x, dim=1, keepdim=True)##对通道进行降维
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        x = torch.cat([avg_out, max_out], dim=1)
        x = self.conv1(x)
        return x0 * self.sigmoid(x)

class dilated_conv(nn.Module):##FEM
    ...
    def forward(self, x):
        ...
        x3 = self.c3(x)
        x4 = self.c4(x)
        xc = torch.cat([x1, x2, x3, x4], axis=1)#按通道连接
        xc = self.c5(xc)
        xs = self.c6(x)
        x = self.relu(xc+xs)
        x = self.sa(x)
...
class SCA(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.ca = nn.Sequential(conv2d(in_c, out_c, kernel_size=1, padding=0), channel_attention(out_c))
        self.cc = conv2d(in_c, out_c, kernel_size=1, padding=0, act=False)
        self.sa = spatial_attention()
...
class TGAPolypSeg(nn.Module):
    def __init__(self):
        ...
        """SCA"""
        self.sca1 = SCA(64,128)
        self.sca2 = SCA(256, 128)
        self.sca3 = SCA(512, 128)
        self.sca4 = SCA(1024, 128)
    ...
    def forward(self, image, label):
        ...
        """ Dilated Conv """
        s1 = self.s1(x1)##FEM,框架中f1-f4对应代码中s1-s4
        s2 = self.s2(x2)
        s3 = self.s3(x3)
        s4 = self.s4(x4)
