...
def calculate_weights(P, Q, beta, T):
    epsilon = 1e-7  # 为了避免除以零
    f = torch.clamp(min(P) / (P - beta * (Q + 1)), min=epsilon)
    w = torch.exp(f / T) / torch.sum(torch.exp(f / T))
    return w.clone().detach().to(dtype=torch.float32)
...
class CustomizedLoss(nn.Module):
    def __init__(self, beta, T):
        ...
        self.beta = beta
        self.T = T
        self.bce_dice_loss = DiceBCELoss()

    def forward(self, predicted_mask, target_mask, num_polyps, polyp_sizes):
        # 确保predicted_mask在[0, 1]范围内
        predicted_mask = torch.sigmoid(predicted_mask)

        P = torch.sum(predicted_mask.view(predicted_mask.size(0), -1), dim=1)
        Q = num_polyps
        weights = calculate_weights(P, Q, self.beta, self.T)
        loss = 0
        for i in range(predicted_mask.size(0)):
            loss += weights[i] * self.bce_dice_loss(predicted_mask[i], target_mask[i])
...
if __name__ == "__main__":
    ...
    seeding(42)
    #######
    dataset_name = 'Kvasir-SEG'  ##dataset:1.Kvasir-SEG  2.CVC-ClinicDB 3.BKAI 4.Kvasir-Sessile
    # Kvasir-SEG
    #######
    ...
    """ Training logfile """
    train_log_path = "files/%s/train_log.txt" % (dataset_name)
    if os.path.exists(train_log_path):
        print("Log file exists")
    else:
        train_log = open("files/%s/train_log.txt" % (dataset_name), "w")
        train_log.write("\n")
        train_log.close()
    ...
    checkpoint_path = "./files/%s/resnet50.pth" % (dataset_name)
    ...
    loss_fn = [CustomizedLoss(beta=2., T=3.), nn.CrossEntropyLoss(), nn.CrossEntropyLoss()]
